...
class NaverLandCrawler:
    def __init__(self, regions, trad_tp_cds=None, rlet_tp_cds=None, tags=None, wprc_max=None, z=None):
        ...
        self.trad_tp_cds = trad_tp_cds or "B1" # 거래 유형 코드 eg. "B1" 전세, "A1" 매매, "B2" 월세
        self.rlet_tp_cds = rlet_tp_cds or "DDDGG,VL" # 매물 유형 코드 eg. "DDDGG" 오피스텔, "VL" 빌라, "APT" 아파트
        self.tags = tags or "TWOROOM,PARKINGYN" # 특정 조건 태그 eg. "TWOROOM" 투룸 매물, "PARKINGYN" 주차 가능 매물
        self.wprc_max = wprc_max or 30000 # 최대 가격 eg. 30000 3억 원
        self.z = z or 13 # 지도 줌 레벨 eg. "13" 구 단위, "14" 동
    ...
    def fetch_listings(self, lat, lon, cortar_no, region_name):
        ...
        detail_url = f"{self.base_url}/cluster/clusterList?view=atcl&cortarNo={cortar_no}&tradTpCd={self.trad_tp_cds}&rletTpCd={self.rlet_tp_cds}&z={self.z}&lat={lat}&lon={lon}&tag={self.tags}&btm={btm}&lft={lft}&top={top}&rgt={rgt}&wprcMax={self.wprc_max}"
    ...
    def fetch_articles(self, listings, cortar_no, region_name):
        ...
        for listing in listings:
            ...
            for idx in range(1, len_pages + 1):
                ...
                list_url = f"{self.base_url}/cluster/ajax/articleList?itemId={lgeo}&lgeo={lgeo}&tradTpCd={self.trad_tp_cds}&rletTpCd={self.rlet_tp_cds}&tag={self.tags}&z={self.z}&lat={lat2}&lon={lon2}&totCnt={count}&cortarNo={cortar_no}&page={idx}&wprcMax={self.wprc_max}"
